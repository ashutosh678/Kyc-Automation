...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center px-3 py-1 text-sm font-medium rounded-full border ${getStatusStyles()}`}>
      {getStatusIcon()}
      {getStatusText()}
    </span>
  );
...
const CompanyDetails = () => {
  ...
  const calculateCompletionStatus = (data) => {
    ...
    const completedFields = requiredFields.filter(field => !!data && !!data[field]).length;
    
    // Calculate percentage
    const percentage = data ? Math.round((completedFields / requiredFields.length) * 100) : 0;
    ...
    return (
      <div className="flex justify-center items-center h-64">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-blue-600 font-medium">Loading document details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 p-8 rounded-xl shadow-md">
        <div className="flex flex-col md:flex-row md:items-center">
          ...
          <div>
            <h3 className="text-xl font-bold text-blue-800 mb-3">No KYC Documents Found</h3>
            <p className="text-blue-700 mb-6 text-lg">
              It looks like you haven't started your KYC verification process yet. Begin by uploading your documents.
            </p>
            <Link
  ...
  const renderDetailSection = (title, data, fileUrl, fieldKey) => {
    ...
    return (
      <div className={`mb-8 border rounded-xl overflow-hidden shadow-sm ${getStatusClass(isCompleted)}`}>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <p className="text-sm font-medium text-gray-500 mb-2">Information</p>
              {data ? (
            ...
            <div>
              <p className="text-sm font-medium text-gray-500 mb-2">Supporting Document</p>
              {fileUrl ? (
                <div className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm flex flex-col">
                  <div className="flex items-center mb-3">
                    ...
                    <p className="text-gray-700 font-medium">Document uploaded</p>
                  </div>
        ...
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 px-6 py-6 text-white">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-blue-100 mt-2 text-lg">Track your verification progress</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button 
                onClick={handleRefresh}
                disabled={refreshing}
                className={`inline-flex items-center bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-lg transition ${refreshing ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
          ...
          <div className="p-6">
              ...
              <>
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                  <div className="mb-4 md:mb-0">
                    ...
                    <p className="text-gray-600">
                      {completionStatus.completedFields} of {completionStatus.requiredFields} required sections completed
                    </p>
                  </div>
                  ...
                  <div className="flex space-x-3">
                    ...
                    {completionStatus.percentage === 100 && (
                      <button
                        className="inline-flex items-center bg-gradient-to-r from-green-600 to-emerald-700 text-white px-4 py-2 rounded-lg font-medium hover:from-green-700 hover:to-emerald-800 transition shadow-md"
                      >
                        <Download size={18} className="mr-2" /> Download Complete Package
                      </button>
                    )}
                    
                    <button
                      className="inline-flex items-center bg-gray-200 text-gray-700 px-3 py-2 rounded-lg font-medium hover:bg-gray-300 transition"
                    >
                ...
                <div className="bg-gray-50 rounded-xl p-6 mb-8 border border-gray-200">
                  <div className="mb-3 flex justify-between items-center">
                    <div>
                      <h3 className="text-base font-semibold text-gray-700">Verification Status</h3>
                      <p className="text-gray-500 text-sm">AI-powered document verification</p>
                    </div>
                    <div>
                      <span className="text-xl font-bold text-blue-600">{completionStatus.percentage}%</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-6">
                    <div className={`p-4 rounded-lg border ${companyDetails.intendedCompanyName ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          {getStatusIcon('intendedCompanyName')}
                          <span className="ml-2 font-medium text-gray-700">Company Name</span>
                        </div>
                      </div>
                    </div>
                    
                    <div className={`p-4 rounded-lg border ${companyDetails.companyActivities ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          {getStatusIcon('companyActivities')}
                          <span className="ml-2 font-medium text-gray-700">Activities</span>
                        </div>
                      </div>
                    </div>
                    
                    <div className={`p-4 rounded-lg border ${companyDetails.intendedRegisteredAddress ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                    ...
                    <div className={`p-4 rounded-lg border ${companyDetails.financialYearEnd ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          {getStatusIcon('financialYearEnd')}
                          <span className="ml-2 font-medium text-gray-700">Financial Year</span>
                        </div>
                      </div>
                    </div>
                    
                    <div className={`p-4 rounded-lg border ${companyDetails.constitution ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          {getStatusIcon('constitution')}
                          <span className="ml-2 font-medium text-gray-700">Constitution</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {activeTab === 'documents' && (
              <div className="space-y-2">
                        ...
                        <div>
                          ...
                          <div className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm">
                            <p className="text-gray-600 mb-2">
                              <span className="font-medium">Option:</span> {companyDetails.constitution.option}
                            </p>
                            <p className="text-gray-800">{companyDetails.constitution.description}</p>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          {companyDetails.constitution.fileId?.fileUrl ? (
                            <div className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm flex flex-col">
                              ...
                              <div className="flex flex-wrap gap-2 mt-2">
                                <a 
                                  href={companyDetails.constitution.fileId.fileUrl} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1.5 rounded-md font-medium transition"
                                >
                ...
                {!companyDetails.constitution && renderDetailSection(
                  'Constitution',
                  null,
                  null,
                  'constitution'
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-green-700 mt-2 text-lg">
                    All required documents have been verified successfully. Your KYC verification process is complete.
                  </p>
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button className="inline-flex items-center bg-green-600 text-white px-4 py-2 rounded-md font-medium hover:bg-green-700 transition shadow-sm">
                      <Download size={18} className="mr-2" /> Download Verification Certificate
                    </button>
          ...
          {completionStatus.percentage < 100 && (
            <div className="p-6 bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-xl shadow-sm">
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="text-xl font-bold text-amber-800">Verification In Progress</h3>
                  <p className="text-amber-700 mt-2 text-lg">
                    Please complete all required sections to finalize your KYC verification.
                    {completionStatus.percentage > 0 && ` You've made progress (${completionStatus.percentage}%), but there's still work to do.`}
                  </p>
                  <div className="mt-4">
                    <Link
                      to={`/company-details-form/${id}`}
